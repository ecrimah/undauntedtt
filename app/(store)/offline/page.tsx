...
export default function OfflinePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-ice via-white to-brand-ice flex items-center justify-center px-6">
      <div className="max-w-md w-full text-center">
        {/* Animated illustration */}
        <div className="relative mb-8">
          <div className="w-32 h-32 mx-auto relative">
            {/* Outer ring */}
            <div className="absolute inset-0 rounded-full border-4 border-brand-caramel/25 animate-pulse" />
            {/* Inner circle */}
            <div className="absolute inset-3 rounded-full bg-gradient-to-br from-brand-ice to-brand-caramel/25 flex items-center justify-center">
              <div className="relative">
                <i className="ri-wifi-off-line text-5xl text-brand-bronze" />
                {/* Animated dots */}
        ...
        <div className="space-y-3 mb-8">
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="w-full bg-brand-bronze hover:bg-brand-caramel disabled:bg-brand-caramel text-white py-4 px-6 rounded-2xl font-semibold transition-all active:scale-[0.98] shadow-lg shadow-brand-bronze/20 flex items-center justify-center gap-2"
          >
            {retrying ? (
              <>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              >
                <i className={`${tip.icon} text-brand-bronze text-sm`} />
                <span>{tip.text}</span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-brand-caramel' : 'bg-red-400'} ${!isOnline && 'animate-pulse'}`} />
