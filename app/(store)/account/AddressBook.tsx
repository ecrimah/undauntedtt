    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="px-4 py-2 bg-brand-bronze text-white rounded-lg font-semibold hover:bg-brand-caramel transition-colors whitespace-nowrap"
        >
      ...
      {showForm && (
        <div className="bg-white border-2 border-brand-bronze rounded-lg p-6 mb-6">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-bronze focus:border-transparent"
            ...
            <div className="md:col-span-2">
              <label className="flex items-center">
                <input type="checkbox" className="w-4 h-4 text-brand-bronze border-gray-300 rounded focus:ring-brand-bronze" />
            ...
            <div className="md:col-span-2 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
              <button
                type="submit"
                className="flex-1 py-3 bg-brand-bronze text-white rounded-lg font-semibold hover:bg-brand-caramel transition-colors whitespace-nowrap"
              >
                Save Address
              </button>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {addresses.map((address) => (
          <div
            key={address.id}
            className={`bg-white border-2 rounded-lg p-6 relative ${address.isDefault ? 'border-brand-bronze' : 'border-gray-200'
              }`}
          >
            {address.isDefault && (
              <div className="absolute top-4 right-4">
                <span className="px-3 py-1 bg-brand-bronze text-white text-xs font-semibold rounded-full whitespace-nowrap">
                  Default
                </span>
              </div>
            )}
