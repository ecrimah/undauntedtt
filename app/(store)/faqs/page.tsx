...
export default function FAQsPage() {
  ...
  const waDigits = (process.env.NEXT_PUBLIC_CONTACT_WHATSAPP || process.env.NEXT_PUBLIC_CONTACT_PHONE || '').replace(/\D/g, '');
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: 'You can modify or cancel your order within 1 hour of placing it. Contact us immediately via the channels on our Contact page. Once an order is processed, modifications may not be possible.'
    },
    {
      category: 'orders',
    ...
    },
    {
      ...
      answer: 'For hygiene reasons, we cannot accept returns on pierced earrings or items worn against broken skin. Custom or personalised pieces are also non-returnable unless defective. All other jewelry is eligible for return within the policy window if unworn and in original packaging.'
    },
    {
      category: 'returns',
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 relative z-20 pb-8">
        <div className="relative bg-white rounded-full shadow-xl border border-gray-100 p-1">
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search for answers..."
            className="w-full pl-12 pr-4 py-4 border-0 rounded-full focus:ring-2 focus:ring-brand-caramel focus:border-transparent text-sm bg-transparent"
          />
          <i className="ri-search-line absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 text-xl" />
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-brand-bronze to-brand-bronze py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            {waDigits ? (
              <a
                href={`https://wa.me/${waDigits.startsWith('0') ? `233${waDigits.slice(1)}` : waDigits}`}
                target="_blank"
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <Link href="/shipping" className="bg-gray-50 p-8 rounded-2xl hover:shadow-lg transition-all cursor-pointer">
            <div className="w-12 h-12 bg-brand-caramel/25 rounded-full flex items-center justify-center mb-4">
              <i className="ri-truck-line text-2xl text-brand-bronze"></i>
            </div>
          ...
          <Link href="/returns" className="bg-gray-50 p-8 rounded-2xl hover:shadow-lg transition-all cursor-pointer">
            <div className="w-12 h-12 bg-brand-caramel/25 rounded-full flex items-center justify-center mb-4">
              <i className="ri-arrow-go-back-line text-2xl text-brand-bronze"></i>
            </div>
