...
function OrderTrackingContent() {
  ...
  const getStatusBadge = () => {
    ...
    const statusMap: Record<string, { label: string; color: string }> = {
      ...
      'processing': { label: 'Processing', color: 'bg-brand-caramel/25 text-brand-bronze' },
      'shipped': { label: 'Packaged', color: 'bg-purple-100 text-purple-800' },
      'delivered': { label: 'Delivered', color: 'bg-brand-caramel/25 text-brand-bronze' },
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <form onSubmit={handleTrack} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-brand-bronze hover:bg-brand-caramel text-white py-4 rounded-lg font-semibold transition-colors whitespace-nowrap disabled:opacity-50"
              >
                {loading ? (
            ...
            <div className="mt-8 p-4 bg-brand-ice border border-brand-caramel/40 rounded-lg">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-brand-bronze">Need Help?</p>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-brand-caramel/25 rounded-full">
                  <i className="ri-map-pin-line text-xl text-brand-bronze"></i>
                </div>
                <div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-brand-caramel/25 rounded-full">
                  <i className="ri-money-cny-circle-line text-xl text-brand-bronze"></i>
                </div>
                <div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-brand-caramel/25 rounded-full">
                  <i className="ri-box-3-line text-xl text-brand-bronze"></i>
                </div>
                <div>
              ...
              <div key={step.key} className="flex items-start mb-8 last:mb-0">
                <div className="relative flex flex-col items-center mr-6">
                  <div className={`w-12 h-12 flex items-center justify-center rounded-full font-bold transition-colors ${
                    step.status === 'completed'
                      ...
                      : step.status === 'active'
                      ? 'bg-brand-caramel/25 text-brand-bronze ring-4 ring-brand-caramel/40'
                  ...
                  {index < trackingSteps.length - 1 && (
                    <div className={`w-0.5 h-16 mt-2 ${
                      step.status === 'completed' ? 'bg-brand-bronze' : 'bg-gray-200'
                    }`}></div>
                  )}
                </div>
                <div className="flex-1 pt-2">
                  ...
                  <p className={`text-sm mt-1 font-semibold ${
                    step.status === 'pending' ? 'text-gray-400' : 'text-brand-bronze'
                  }`}>
                    {step.date}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            {order.order_items?.map((item: any) => (
              <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="font-bold text-brand-bronze">GH₵ {Number(item.unit_price).toFixed(2)}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <Suspense fallback={
      <main className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-brand-bronze border-t-transparent rounded-full animate-spin mx-auto"></div>
        </div>
      </main>
    }>
